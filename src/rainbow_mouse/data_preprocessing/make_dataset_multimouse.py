...
y1 = natural_scenes['frame'].astype(int).values 
...
y2 = natural_scenes['frame'].astype(int).values  # [n_trials]
y= np.concatenate((y1, y2), axis=0)
eta = 1e-6
nan_mask = np.isnan(X)
print('nans', np.sum(nan_mask))

X[nan_mask] = np.random.normal(loc=0.0, scale=eta, size=nan_mask.sum())
# Save
np.save(f"{rainbow_path}/lfp_multi_mouse.npy", X)
np.save(f"{rainbow_path}/frames_multi_mouse.npy", y)
